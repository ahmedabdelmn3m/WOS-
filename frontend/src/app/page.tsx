...
function calcDamage(atk: number, atkPct: number, leth: number, lethPct: number) {
  return (atk * (1 + atkPct / 100) * leth * (1 + lethPct / 100)) / 100
}
function calcDefense(def: number, defPct: number, hp: number, hpPct: number) {
  return (def * (1 + defPct / 100) * hp * (1 + hpPct / 100)) / 100
}
function effectiveDmg(raw: number, defense: number) {
...
const TYPE_ADV: Record<TroopKey, TroopKey> = {
  infantry: 'lancer', lancer: 'marksman', marksman: 'infantry'
}

function simulateBattle(user: ArmyStats, enemy: ArmyStats): BattleResult {
  const troops: TroopKey[] = ['infantry', 'lancer', 'marksman']
  const get = (a: ArmyStats, t: keyof ArmyStats) => a[t]

  const uDmg = Object.fromEntries(troops.map(t => [t, calcDamage(get(user,t).atk, get(user,t).atkPct, get(user,t).leth, get(user,t).lethPct)]))
  const eDmg = Object.fromEntries(troops.map(t => [t, calcDamage(get(enemy,t).atk, get(enemy,t).atkPct, get(enemy,t).leth, get(enemy,t).lethPct)]))
  const uDef = Object.fromEntries(troops.map(t => [t, calcDefense(get(user,t).def, get(user,t).defPct, get(user,t).hp, get(user,t).hpPct)]))
  const eDef = Object.fromEntries(troops.map(t => [t, calcDefense(get(enemy,t).def, get(enemy,t).defPct, get(enemy,t).hp, get(enemy,t).hpPct)]))

  const uHP: Record<string,number> = { infantry:100, lancer:100, marksman:100 }
  const eHP: Record<string,number> = { infantry:100, lancer:100, marksman:100 }
  let round = 0

  while (round < 20) {
    round++
    const uA = troops.filter(t => uHP[t] > 0)
    const eA = troops.filter(t => eHP[t] > 0)
    if (!uA.length || !eA.length) break

    uA.forEach(t => {
      const live = eA.filter(x => eHP[x] > 0)
      if (!live.length) return
      const pref = TYPE_ADV[t]
      const tgt = live.includes(pref) ? pref : live[0]
      let d = effectiveDmg(uDmg[t], eDef[tgt])
      if (tgt === pref) d *= 1.15
      eHP[tgt] = Math.max(0, eHP[tgt] - (d * 0.8) / round)
    })
    eA.forEach(t => {
      const live = uA.filter(x => uHP[x] > 0)
      if (!live.length) return
      const pref = TYPE_ADV[t]
      const tgt = live.includes(pref) ? pref : live[0]
  ...
  const uDT = Object.values(uDmg).reduce((a,b)=>a+b,0)
  const eDT = Object.values(eDmg).reduce((a,b)=>a+b,0)
  const uDF = Object.values(uDef).reduce((a,b)=>a+b,0)
  const eDF = Object.values(eDef).reduce((a,b)=>a+b,0)
  const ratio = (uDT + uDF) / Math.max(eDT + eDF, 0.001)
  const uAvg = Object.values(uHP).reduce((a,b)=>a+b,0) / 3
  const eAvg = Object.values(eHP).reduce((a,b)=>a+b,0) / 3
  const win = Math.min(97, Math.max(3, Math.round(50 + (ratio-1)*40 + (uAvg-eAvg)*0.3)))

  const labels: Record<string,string> = { atk:'ATK', def:'DEF', hp:'HP', leth:'LETH' }
  const tl: Record<string,string> = { infantry:'Infantry', lancer:'Lancer', marksman:'Marksman' }
  let bigAdv='', advD=-Infinity, bigWeak='', weakD=Infinity
  troops.forEach(t => {
    (['atk','def','hp','leth'] as const).forEach(s => {
      const pKey = s + 'Pct' as keyof TroopStats
      const uv = (user[t] as any)[s] * (1 + (user[t] as any)[pKey] / 100)
      const ev = (enemy[t] as any)[s] * (1 + (enemy[t] as any)[pKey] / 100)
      const d = uv - ev
      const n = `${tl[t]} ${labels[s]}`
      if (d > advD) { advD=d; bigAdv=n }
      if (d < weakD) { weakD=d; bigWeak=n }
    })
  })

  const tot = troops.reduce((a,t)=>a+uDmg[t]+uDef[t],0)||1
  const form = Object.fromEntries(troops.map(t=>[t, Math.round((uDmg[t]+uDef[t])/tot*100)]))
  form.marksman += 100 - Object.values(form).reduce((a,b)=>a+b,0)

  return {
    winProbability: win, winner: win>=50?'user':'enemy', roundsFought: round,
    userLossesPct: Object.fromEntries(troops.map(t=>[t, Math.round(Math.max(0,100-uHP[t]))])),
    enemyLossesPct: Object.fromEntries(troops.map(t=>[t, Math.round(Math.max(0,100-eHP[t]))])),
    biggestAdvantage: bigAdv, biggestWeakness: bigWeak,
    recommendedFormation: form,
    userDamageScore: uDT, enemyDamageScore: eDT,
    userDefenseScore: uDF, enemyDefenseScore: eDF,
  }
}
...
function reverseOptimize(user: ArmyStats, enemy: ArmyStats, targets=[75,85,95]) {
  ...
  return targets.map(pct => {
    if (cur >= pct) return { targetPct:pct, achieved:true, current:cur, upgrades:[] }
    const upgrades: any[] = []
    const troops: TroopKey[] = ['infantry','lancer','marksman']
    const stats = [
      {key:'atk',step:200,label:'Attack'},{key:'leth',step:200,label:'Lethality'},
      {key:'def',step:300,label:'Defense'},{key:'hp',step:300,label:'Health'},
    ]
    const tl: Record<string,string> = { infantry:'Infantry', lancer:'Lancer', marksman:'Marksman' }
    troops.forEach(t => {
...
function StatEditor({ stats, onChange }: { stats: ArmyStats, onChange: (t:string,k:string,v:number)=>void }) {
  return (
    <div>
      {TROOPS.map(t => (
        <div key={t.key} style={{background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:8,padding:14,marginBottom:10}}>
          <div style={{fontFamily:'sans-serif',fontWeight:700,letterSpacing:2,fontSize:12,textTransform:'uppercase',color:t.color,marginBottom:10}}>{t.label}</div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8}}>
            {FIELDS.map(f => (
              <div key={f.key}>
                ...
                <input
                  type="number"
                  value={(stats[t.key as keyof ArmyStats] as any)[f.key]}
...
export default function Home() {
  ...
  function updateUser(troop:string,key:string,val:number) {
    setPresets(ps=>ps.map(p=>p.id!==activeId?p:{...p,stats:{...p.stats,[troop]:{...p.stats[troop as keyof ArmyStats],[key]:val}}}))
    setResult(null); setOptimizer(null)
  }
  function updateEnemy(troop:string,key:string,val:number) {
    setEnemyStats(s=>({...s,[troop]:{...s[troop as keyof ArmyStats],[key]:val}}))
    setResult(null); setOptimizer(null)
  }
  function runSim() {
  ...
  function runOptimizer() {
    const r = reverseOptimize(active.stats, enemyStats)
    setOptimizer(r)
  }
  ...
  const barColor = result ? (result.winProbability>=65?'#40d090':result.winProbability>=45?'#f0b840':'#f04040') : '#4dd9f0'
  const accuracy = feedback.length ? Math.round(feedback.filter(f=>f.correct).length/feedback.length*100) : null

  const navStyle = (t:string) => ({
    flex:1, padding:'8px 4px', background: tab===t?'var(--bg-card)':'none',
  ...
  const S = {
    panel: {background:'var(--bg-panel)',border:'1px solid var(--border)',borderRadius:10,padding:20} as React.CSSProperties,
    title: {fontFamily:'sans-serif',fontWeight:700,fontSize:15,letterSpacing:2,color:'var(--accent-ice)',textTransform:'uppercase' as const,marginBottom:16},
    label: {fontSize:10,color:'var(--text-muted)',letterSpacing:1,textTransform:'uppercase' as const,marginBottom:6,display:'block'},
    btn: (color='ice') => ({
      ...
      border1: color==='ghost'?'1px solid var(--border-bright)':'none',
    }),
    card: {background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:8,padding:12} as React.CSSProperties,
  }
  ...
  return (
    <main style={{maxWidth:1000,margin:'0 auto',padding:16}}>
      ...
      <div style={{textAlign:'center',padding:'20px 0 16px',borderBottom:'1px solid var(--border)',marginBottom:4}}>
        <h1 style={{fontFamily:'sans-serif',fontWeight:900,fontSize:24,letterSpacing:4,color:'var(--accent-ice)',textTransform:'uppercase'}}>⚔ Whiteout Survival</h1>
      ...
      <div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 0',fontSize:12}}>
        ...
        <span style={{background:'rgba(77,217,240,.12)',color:'var(--accent-ice)',border:'1px solid rgba(77,217,240,.25)',padding:'2px 8px',borderRadius:4,fontSize:10,fontWeight:700,letterSpacing:1}}>{active?.name}</span>
        {accuracy!==null && <span style={{marginLeft:'auto',color:'var(--text-dim)',fontSize:11}}>Accuracy: <span style={{color:'var(--accent-gold)'}}>{accuracy}%</span> ({feedback.length} battles)</span>}
      </div>
      ...
      {tab==='battle' && (
        <div style={S.panel}>
          <div style={S.title}>⚔ Battle Simulator</div>
          <div style={{display:'flex',gap:6,marginBottom:12,flexWrap:'wrap'}}>
            {presets.map(p=>(
              <button key={p.id} onClick={()=>{setActiveId(p.id);setResult(null)}}
                style={{...S.btn(p.id===activeId?'ice':'ghost'),fontSize:10,padding:'5px 12px'}}>
                {p.name}
              </button>
            ))}
          </div>
          <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap'}}>
            <button style={S.btn('fire')} onClick={runSim}>⚡ Run Battle Simulation</button>
            {result && <button style={S.btn('gold')} onClick={runOptimizer}>◈ What Do I Need to Win?</button>}
          </div>
          ...
          {result && (
            <div style={{background:'var(--bg-dark)',border:'1px solid var(--accent-ice)',borderRadius:10,padding:20}}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}>
                <div>
                  <span style={{fontSize:10,color:'var(--text-muted)',letterSpacing:2,textTransform:'uppercase'}}>Win Probability</span>
                ...
                <span style={{padding:'4px 12px',borderRadius:4,fontSize:12,fontWeight:700,letterSpacing:1,
                ...
                {!feedbackDone ? (
                  <>
                    ...
                    <div style={{display:'flex',gap:8,marginBottom:10}}>
                      {['yes','no'].map(opt=>(
                        <div key={opt} onClick={()=>setFeedbackMode(opt)} style={{flex:1,padding:12,borderRadius:8,cursor:'pointer',textAlign:'center',
                    ...
                    {feedbackMode && (
                      <button style={S.btn('ghost')} onClick={submitFeedback}>Submit Feedback</button>
                    )}
                  </>
                ) : (
          ...
          {optimizer && (
            <div style={{...S.panel,marginTop:12}}>
              <div style={S.title}>◈ Reverse Optimizer</div>
              {optimizer.map(r => (
                <div key={r.targetPct} style={{background:'var(--bg-card)',border:`1px solid var(--border)`,borderLeft:`3px solid ${r.targetPct===75?'var(--accent-gold)':r.targetPct===85?'var(--accent-ice)':'var(--accent-green)'}`,borderRadius:8,padding:14,marginBottom:10}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                    ...
                    {r.achieved ? <span style={{fontSize:10,color:'var(--accent-green)',fontWeight:700}}>✓ Already Achieved</span>
                      : <span style={{fontSize:11,color:'var(--text-muted)'}}>Current: {r.current}%</span>}
                  </div>
                  {!r.achieved && r.upgrades.map((u:any,i:number)=>(
      ...
      {tab==='enemy' && (
        <div style={S.panel}>
          <div style={S.title}>👁 Enemy Scout Input</div>
          ...
          <div style={{display:'flex',gap:8,marginTop:16}}>
            <button style={S.btn('fire')} onClick={()=>{setTab('battle');runSim()}}>⚡ Save & Run Simulation</button>
            <button style={{...S.btn('ghost'),fontSize:10}} onClick={()=>setEnemyStats(mkArmy(1.05))}>Reset</button>
          </div>
        </div>
      )}
      ...
      {tab==='presets' && (
        <div>
          <div style={S.panel}>
            <div style={S.title}>📋 My Presets</div>
            <div style={{display:'flex',gap:8,marginBottom:16}}>
              ...
              <button style={S.btn('ice')} onClick={createPreset}>+ Create</button>
            </div>
            {presets.map(p=>(
              <div key={p.id} onClick={()=>setActiveId(p.id)}
                style={{background:'var(--bg-card)',border:`1px solid ${p.id===activeId?'var(--accent-ice)':'var(--border)'}`,borderRadius:8,padding:14,marginBottom:8,cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',background2:p.id===activeId?'rgba(77,217,240,.06)':'var(--bg-card)'}}>
                <div>
                  <div style={{fontWeight:700,fontSize:14}}>{p.name} {p.id===activeId&&<span style={{fontSize:9,color:'var(--accent-ice)',marginLeft:6,border:'1px solid var(--accent-ice)',padding:'1px 5px',borderRadius:3}}>ACTIVE</span>}</div>
                  <div style={{fontSize:10,color:'var(--text-muted)',fontFamily:'monospace',marginTop:2}}>INF ATK: {p.stats.infantry.atk.toLocaleString()} | LAN DEF: {p.stats.lancer.def.toLocaleString()} | MRK HP: {p.stats.marksman.hp.toLocaleString()}</div>
                </div>
                {presets.length>1 && (
          ...
          <div style={{...S.panel,marginTop:12}}>
            <div style={S.title}>✏ Edit: {active?.name}</div>
      ...
      {tab==='history' && (
        <div style={S.panel}>
          <div style={S.title}>📊 Feedback History</div>
          {accuracy!==null && (
          ...
          {feedback.length>0 && (
            <button style={{...S.btn('ghost'),marginTop:12,color:'var(--accent-red)',border:'1px solid rgba(240,64,64,.3)',fontSize:10}} onClick={()=>setFeedback([])}>
              Clear History
            </button>
          )}
        </div>
